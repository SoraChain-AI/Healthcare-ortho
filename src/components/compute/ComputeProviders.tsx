  ...
  return <div className="space-y-8">
      <div className="text-center space-y-4">
        <div className="space-y-2">
          <h2 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-slate-800 via-slate-600 to-slate-800 dark:from-slate-100 dark:via-slate-300 dark:to-slate-100 bg-clip-text text-transparent">Connect Compute Providers</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {providers.map(provider => <Card key={provider.id} className={`glass-card cursor-pointer floating-element border-0 backdrop-blur-xl bg-white/10 dark:bg-black/20 border border-white/20 dark:border-white/10 ${selectedProvider === provider.id ? 'ring-2 ring-blue-400/50 shadow-xl shadow-blue-500/20' : 'hover:bg-white/20 dark:hover:bg-black/30'} ${isConnected(provider.id) ? 'ring-2 ring-green-400/50 shadow-xl shadow-green-500/20' : ''} transition-all duration-500 hover:scale-105 hover:shadow-2xl relative overflow-hidden`} onClick={() => handleProviderSelect(provider.id)}>
            {isConnected(provider.id) && <div className="absolute top-4 right-4 z-10">
            ...
            <CardHeader className="space-y-4 p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {provider.features.map((feature, index) => <span key={index} className="text-xs bg-slate-100/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 px-2 py-1 rounded-md border border-white/20">
                      {feature}
                    </span>)}
                </div>
              </div>
              ...
              <div className="pt-2">
                {isConnected(provider.id) ? <div className="flex items-center gap-2 text-green-600 dark:text-green-400 text-sm font-medium">
                    <CheckCircle className="h-4 w-4" />
                    <span>Connected & Ready</span>
                  </div> : <Button onClick={e => handleQuickConnect(provider.id, e)} className="w-full glass-button bg-gradient-to-r from-emerald-500 to-cyan-500 text-white border-0 px-6 py-3 glow-effect hover:from-emerald-600 hover:to-cyan-600 transition-all duration-200 group">
