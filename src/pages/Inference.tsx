...
const VALID_TYPES = ['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'image/jpeg', 'image/png'];
const Inference = () => {
  const {
    toast
  } = useToast();
  ...
  return <Layout>
      <SectionHeader title="Knee Specialist - Orthopedic" description="Upload your reports or x-rays for expert AI-powered assessment" icon={<Upload className="h-8 w-8" />} />
        ...
        <Card className="glass-card max-w-2xl w-full backdrop-blur-lg border-2 border-sorachain-light/30 shadow-xl relative animate-fade-in">
          ...
          <CardContent>
            <div className={`transition-all duration-300 p-0`}>
              <div onDrop={handleDrop} onDragOver={e => e.preventDefault()} className="border-2 border-dashed border-sorachain-light hover:border-sorachain-primary transition rounded-2xl bg-white/60 dark:bg-sorachain-dark/50 flex flex-col items-center justify-center p-8 min-h-[200px] glass-card glow-effect cursor-pointer" onClick={handleUploadClick} style={{
              boxShadow: '0 12px 45px 0 rgba(102,134,255,0.12), 0 1.5px 4px 0 rgba(102,134,255,0.15)'
            }} tabIndex={0} role="button" aria-label="Upload medical reports">
                ...
                <Input id="file-upload" type="file" className="hidden" multiple accept=".pdf,.docx,.jpg,.jpeg,.png,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/jpeg,image/png" onChange={handleFileChange} />
              </div>
              {fileUploadError && <div className="bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-200 p-3 rounded-md text-sm mt-3 flex items-center">
                  <AlertCircle className="h-4 w-4 mr-2" />
                  {fileUploadError}
                </div>}
            </div>
            {uploadedFiles.length > 0 && <div className="bg-gray-50/60 dark:bg-gray-800/60 rounded-lg mt-6 p-4 animate-fade-in">
                <h3 className="text-base font-bold mb-2 text-sorachain-primary">Files ready for consultation:</h3>
                <ul className="space-y-2">
                  {uploadedFiles.map((file, idx) => <li key={idx} className="flex justify-between items-center bg-white/70 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 px-3 py-2 rounded-md">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => removeFile(idx)} className="h-7 w-7 p-0" title="Remove">
                        <X className="h-4 w-4" />
                      </Button>
                    </li>)}
                </ul>
            ...
            <div className="pt-8 space-y-3">
              <Button onClick={handleSubmit} className="w-full sorachain-button transition-all duration-300 text-lg font-bold shadow-md py-4" disabled={isUploading}>
                {isUploading ? <><Loader2 className="h-5 w-5 mr-2 animate-spin" /> Uploading...</> : 'Submit for Consultation'}
              </Button>
